...
<div class="flex flex-wrap">
    <div class="w-full my-6 pr-0 lg:pr-2">
        <div class="leading-loose">
            <form class="p-10 bg-white rounded shadow-xl" action="{{ route('lessons.destroy', [$module->id, $lesson->id]) }}" method="POST">
                <ul>
                    <li class="m-3"><strong>Nome: </strong>{{ $lesson->name }}</li>
                    <li class="m-3"><strong>URL: </strong>{{ $lesson->url }}</li>
                    <li class="m-3"><strong>Vídeo: </strong>{{ $lesson->video }}</li>
                    <li class="m-3"><strong>Descrição: </strong>{{ $lesson->description }}</li>
                </ul>
                @method('DELETE')
                @csrf
                <div class="mt-6">
    ...
    @include('admin.courses.modules.lessons.partials.back-index')
</div>
@endsection
