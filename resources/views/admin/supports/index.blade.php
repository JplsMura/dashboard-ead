...
@section('content')
        ...
        <form action="" method="get" class="w-full max-w-lg">
            <div class="flex flex-wrap -mx-3 mb-6">
                ...
                <div class="w-full md:w-1/2">
                    <button type="submit" class="shadow bg-purple-500 hover:bg-purple-400 focus:shadow-outline focus:outline-none text-white font-bold py-4 px-4 rounded">
                        Filtrar
                    </button>
                </div>
            </div>
    ...
    <nav aria-label="Page navigation py-12">
        <ul class="mt-4 inline-flex -space-x-px">
            @if ($supports->currentPage() > 1)
                <li>
                    <a href="?page={{ $supports->currentPage() - 1 }}&status={{ request('status', 'P') }}"
            ...
            <li>
                <a href="#" aria-current="page"
                    class="py-2 px-3 text-blue-600 bg-blue-50 border border-gray-300 hover:bg-blue-100 hover:text-blue-700 dark:border-gray-700 dark:bg-gray-700 dark:text-white">{{ $supports->currentPage() }}</a>
            </li>
            @if ($supports->currentPage() < $supports->lastPage())
                <li>
                    <a href="?page={{ $supports->currentPage() + 1 }}&status={{ request('status', 'P') }}"
                        class="py-2 px-3 leading-tight text-gray-500 bg-white rounded-r-lg border border-gray-300 hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">Próxima</a>
                </li>
            @endif
        </ul>
    </nav>
